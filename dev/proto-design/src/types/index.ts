...
export interface Player {
  ...
  maxSkillSlots: number;            // 최대 스킬 슬롯 수 (기본 4, 최대 6)
  // 클래스별 메커니즘
...
// 배경 테마 키 (지역별 배경 이미지 매핑용)
export type BgTheme = 'forest' | 'castle' | 'dungeon';
...
export interface Region {
  ...
  bgTheme: BgTheme;       // 배경 테마 (배경 이미지 전환용)
}
...
export interface EventEffect {
  type: 'heal' | 'damage' | 'soul_gain' | 'soul_cost' | 'info';
  value: number;           // 고정값 또는 최솟값
  maxValue?: number;       // 범위 지정 시 최댓값
}

// 이벤트 선택지의 확률 분기
export interface EventOutcome {
  chance: number;           // 0~1 (예: 0.7 = 70%)
  label: string;            // "성공" | "실패"
  effects: EventEffect[];
...
export interface EventChoice {
  ...
  costDescription?: string; // 비용 표시 (예: "소울 8")
  outcomes: EventOutcome[]; // 확률 분기 (1개면 확정, 2개면 확률)
}
...
export interface EventDefinition {
  ...
  pool: 'global' | 'forest'; // 전역 풀 또는 지역 전용 풀
  narration: string;         // 나레이션 텍스트
  choices: EventChoice[];
  canAbandon: boolean;       // 포기 가능 여부 (A, C만 true)
}

// 이벤트 결과 (선택 후 표시)
export interface EventResult {
  choiceId: string;
  outcomeIndex: number;      // outcomes 배열 내 인덱스
  effects: EventEffect[];
  description: string;
}

// 이벤트 진행 상태
export type EventPhase = 'narration' | 'choosing' | 'coin_flip' | 'result';
...
export interface ShopState {
  ...
  pendingPurchaseItemId?: string;
  replacingSkillItemId?: string;
}
