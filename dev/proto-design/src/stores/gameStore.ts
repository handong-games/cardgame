...
import type { GameState, CharacterClass, Player, RunState, CoinTossResult, Skill, EventEffect } from '../types';
...
interface GameActions {
  ...
  skipBloodAltar: () => void;
  openShop: () => void;
  purchaseShopItem: (itemId: string) => { success: boolean; needsReplace?: boolean; reason?: string };
  replaceSkillForShop: (oldSkillId: string) => void;
  cancelReplace: () => void;
  closeShop: () => void;
...
export const useGameStore = create<GameState & GameActions>((set, get) => ({
  ...
  purchaseShopItem: (itemId: string): { success: boolean; needsReplace?: boolean; reason?: string } => {
    ...
    if (!shop) return { success: false, reason: '상점이 열려있지 않습니다' };

    const item = shop.items.find(i => i.id === itemId);
    if (!item || item.sold) return { success: false, reason: '이미 판매된 상품입니다' };
    if (player.souls < item.price) return { success: false, reason: '소울이 부족합니다' };

    if (item.type === 'skill' && item.skill) {
      if (player.skills.length >= player.maxSkillSlots) {
        set({
          shop: { ...shop, pendingPurchaseItemId: itemId },
        });
        return { success: false, needsReplace: true };
      }

      const newSouls = player.souls - item.price;
      const newSkills = [...player.skills, item.skill];
      const newSkillStates = [...player.skillStates, { skillId: item.skill.id, usedThisTurn: 0, cooldownRemaining: 0 }];
    ...
    if (item.type === 'loot' && item.loot) {
      const newSouls = player.souls - item.price;
      const newLoots = [...run.loots, item.loot];
      const newItems = shop.items.map(i => i.id === itemId ? { ...i, sold: true } : i);
      const dialogue = getRandomDialogue('purchase');
    ...
    if (item.type === 'slot_expansion') {
      if (player.maxSkillSlots >= 6) return { success: false, reason: '이미 최대 슬롯입니다' };
    ...
    return { success: false, reason: '알 수 없는 상품 타입입니다' };
  },
  ...
  replaceSkillForShop: (oldSkillId: string) => {
    ...
    if (!item || !item.skill || item.sold) return;
    if (player.souls < item.price) return;

    const newSouls = player.souls - item.price;
    const newSkills = player.skills
      .filter(s => s.id !== oldSkillId)
      .concat(item.skill);
    const newSkillStates = player.skillStates
      .filter(ss => ss.skillId !== oldSkillId)
      .concat({ skillId: item.skill.id, usedThisTurn: 0, cooldownRemaining: 0 });
    const newItems = shop.items.map(i => i.id === item.id ? { ...i, sold: true } : i);
    const dialogue = getRandomDialogue('purchase');
  ...
  cancelReplace: () => {
    const { shop } = get();
    if (!shop) return;
    set({ shop: { ...shop, pendingPurchaseItemId: undefined } });
  },
  ...
  openEvent: (eventId?: string) => {
    const eventDef = eventId ? getEventById(eventId) : undefined;
    const event = eventDef ?? getRandomEvent('forest');

    set({
      battle: { ...get().battle, phase: 'event' as const },
      event: {
        event,
        phase: 'narration',
        result: null,
  ...
  selectEventChoice: (choiceId: string) => {
    ...
    for (let i = 0; i < choice.outcomes.length; i++) {
      cumulative += choice.outcomes[i].chance;
      if (roll < cumulative) {
    ...
    const outcome = choice.outcomes[selectedOutcomeIndex];
    const resolvedEffects: EventEffect[] = outcome.effects.map(effect => {
      if (effect.maxValue !== undefined && effect.maxValue > effect.value) {
        const resolved = effect.value + Math.floor(Math.random() * (effect.maxValue - effect.value + 1));
        return { ...effect, value: resolved, maxValue: undefined };
      }
      return effect;
    });
    ...
    for (const effect of resolvedEffects) {
      switch (effect.type) {
        case 'heal':
          ...
          break;
        case 'damage':
          newPlayer.hp = Math.max(1, newPlayer.hp - effect.value);
          break;
        case 'soul_gain':
          newPlayer.souls += effect.value;
          break;
        case 'soul_cost':
    ...
    const needsCoinFlip = choice.outcomes.length > 1;
