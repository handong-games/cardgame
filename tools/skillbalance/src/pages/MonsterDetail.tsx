  ...
  if (!monster || !details) {
    return (
      <div className="min-h-screen bg-white px-6 py-10 text-slate-900">
        <div className="mx-auto max-w-4xl rounded-3xl border border-slate-200 bg-white/80 p-6">
          <p className="text-sm text-slate-500">몬스터를 찾을 수 없습니다.</p>
          <Link
            to="/"
            className="mt-4 inline-flex rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-700"
          >
            돌아가기
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#fff3d6,_#fdecef,_#e9f6ff)] px-6 py-10 text-slate-900">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
        <header className="rounded-3xl border border-slate-200 bg-white/80 p-8">
          <Link
            to="/"
          ...
          <p className="mt-2 text-xs text-slate-500">
            플레이어 HP는 기본 60, 방어/회복 미적용 기준으로 계산된다.
          </p>
        </header>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white/80 p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-2xl bg-white/60 p-4">
              <p className="text-xs text-slate-500">피해감소 단계</p>
              <p className="mt-2 text-2xl font-semibold">
                {monster.damageReduceStage}
              </p>
            </div>
            <div className="rounded-2xl bg-white/60 p-4">
              <p className="text-xs text-slate-500">패턴 길이</p>
              <p className="mt-2 text-2xl font-semibold">
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white/60 p-4">
            <p className="text-xs text-slate-500">턴별 행동</p>
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              {patternRows.map((row) => (
                <li key={row.turn} className="flex flex-col gap-1 md:flex-row">
                  <span className="text-xs text-slate-500">T{row.turn}</span>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-slate-200 bg-white/80 p-6">
            ...
            <p className="mt-2 text-xs text-slate-500">사용 조합</p>
            <p className="text-sm text-slate-900">{minScenario.comboSummary}</p>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/80 p-6">
            ...
            <p className="mt-2 text-xs text-slate-500">사용 조합</p>
            <p className="text-sm text-slate-900">{maxScenario.comboSummary}</p>
            <div className="mt-4 rounded-2xl bg-white/70 p-4">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white/80 p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div>
              <label className="text-sm text-slate-600">코인 시나리오</label>
              <select
                value={coinScenario}
                onChange={(event) =>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white/80 p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white/60 p-4">
              <p className="text-xs text-slate-500">최소 턴 진행</p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white/60 p-4">
              <p className="text-xs text-slate-500">최대 턴 진행</p>
              <ul className="mt-3 space-y-3 text-sm text-slate-700">
