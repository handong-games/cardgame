...
function createPortraitFramePrompt(subject: string, accentLines: string[], detailLines: string[], outputLine: string) {
  return joinPrompt(
    ...
    'aged parchment cream outer frame #F0E8D8',
    'parchment texture inner structure with warm cream base',
    'soft rounded corners 10 to 12px radius',
    'minimal design with color border only and no corner decorations',
    'exact full-card frame silhouette matching a clean 2:3 card ratio from top to bottom',
    'thin rose-tinted inner band should trace the full card frame proportion evenly on all sides',
    'bottom border thickness and side border thickness should remain visually identical',
    ...accentLines,
    'inner portrait window softly lit with warm cream center',
    'inside the card frame should stay visually empty and open with no extra plaques, bars, boxes, or inset panels except the bottom nameplate',
    'very subtle cream-tone vignette at edges',
    'smooth center-to-edge transition that keeps the portrait readable at card size',
    'bottom parchment ribbon nameplate should sit on top of the card frame like an overlay plaque',
    'nameplate should sit slightly lower, close to the bottom edge, while still letting a small section of the lower inner rose band remain visible above the ribbon centerline',
    'nameplate may extend slightly beyond the lower frame boundary while remaining centered',
    'soft folded ribbon ends and gentle paper overlap at both sides of the nameplate',
    'centered nameplate panel occupying roughly 13 to 15 percent of card height',
    'nameplate area kept clean and readable for character or monster name text placement',
    ...detailLines,
    'clean empty center area for portrait insertion',
    'aside from the bottom nameplate overlap, area outside the outer frame edge must remain solid white with no shadow, glow, texture, or color spill',
...
function createSkillFramePrompt(accentLines: string[], detailLines: string[]) {
  return joinPrompt(
    ...
    ...accentLines,
    'centered square icon well optimized for 140x140 in-game usage',
    'icon well should be the only visible inner frame shape and occupy about 70 to 76 percent of the canvas width',
    'icon well should use the same warm parchment material family as the rest of the frame and must not become a separate white panel',
    'inside the icon well should remain naturally empty with no white square insert, no center box, and no extra framing',
    'outside the icon well keep the structure minimal and quiet with no extra panels or decorative inserts',
    'subtle inner glow around icon well for readability at small size',
    'very subtle cream-tone vignette at edges',
    ...detailLines,
...
const UI_BUTTON_STYLE = joinPrompt(
  ...
  'parchment interface element with subtle warm gradient depth',
  'rounded rectangle shape with soft aged paper edges',
  'cream parchment base #F0E8D8 with gold accent #C9A86C',
  ...
  'frame': [
    {
      ...
      prompt: createPortraitFramePrompt(
        ...
        [
          'soft rose-quartz glow accent #E8B4B8',
          'soft upper-left warm rim light matching cozy lavender world lighting',
          'soft rose-quartz tinted lining #E8B4B8',
        ],
        [
          'parchment and cream paper material cues in the outer corners',
    ...
    },
    {
      ...
      prompt: createPortraitFramePrompt(
        ...
        [
          'soft muted-silver glow accent #B8B8C8',
          'soft upper-left warm rim light matching the player frame lighting',
          'soft muted-silver tinted lining #B8B8C8',
        ],
        [
          'same exact frame silhouette, border proportions, and ribbon structure as the player card frame from assets/frames/frame-player.png',
          'keep the full player frame shape unchanged and only swap the rose-tinted band to muted silver #B8B8C8',
          'clean practical silhouette with no ornamental hierarchy marks beyond a quiet metallic glow',
          'designed for ordinary monsters that still belong to the same cozy pastel card world as the player',
          'friendly readable tone for common enemies, never luxurious',
        ],
        'output resolution 512x682 pixels, final usage 180x240 pixels after downscaling',
      ),
      negative: FRAME_NEGATIVE
    },
    {
      ...
      prompt: createPortraitFramePrompt(
        ...
        [
          'soft muted-gold glow accent #C8B888',
          'soft upper-left warm rim light matching the player frame lighting',
          'soft muted-gold tinted lining #C8B888',
        ],
        [
          'same exact frame silhouette, border proportions, and ribbon structure as the player card frame from assets/frames/frame-player.png',
          'keep the full player frame shape unchanged and only swap the rose-tinted band to muted gold #C8B888',
          'slightly deeper inner shadow than tier 1 while keeping the same overall frame form',
          'elevated but still cozy aura suggesting stronger threat without horror',
          'minimalist elite treatment with no excessive crown motifs or ornate spikes',
        ],
        'output resolution 512x682 pixels, final usage 180x240 pixels after downscaling',
      ),
      negative: `${FRAME_NEGATIVE},
cute friendly, cheerful bright`
    },
    {
      ...
      prompt: createPortraitFramePrompt(
        ...
        [
          'soft muted-rose glow accent #C89098 with restrained warm undertone',
          'soft upper-left warm rim light matching the player frame lighting',
          'soft muted-rose tinted lining #C89098',
        ],
        [
          'same exact frame silhouette, border proportions, and ribbon structure as the player card frame from assets/frames/frame-player.png',
          'keep the full player frame shape unchanged and only swap the rose-tinted band to muted rose #C89098',
          'heaviest frame weight among the enemy tiers with deeper corner shadow pockets',
          'important boss presence without horror, with ancient authority and clean readability',
          'add a restrained boss-only accent such as small crown-like upper crest notches or subtle ancient corner insets',
          'minimalist boss treatment with weathered majesty and no over-decorated spikes',
        ],
        'output resolution 512x682 pixels, final usage 180x240 pixels after downscaling',
      ),
      negative: `${FRAME_NEGATIVE},
...
scary dark menacing`
    },
    {
      ...
      prompt: createSkillFramePrompt(
        [
          'muted pastel-rose glow accent #D4A0A0',
          'subtle pastel-rose rim light effect around the border',
        ],
        [
    ...
    },
    {
      ...
      prompt: createSkillFramePrompt(
        [
          'muted pastel-sky glow accent #A0B8D4',
          'subtle pastel-sky rim light effect around the border',
        ],
        [
    ...
    },
    {
      ...
      prompt: createSkillFramePrompt(
        [
          'muted pastel-sage glow accent #A0C8A0',
          'subtle pastel-sage rim light effect around the border',
        ],
        [
    ...
    },
    {
      ...
      prompt: createSkillFramePrompt(
        [
          'muted pastel-lavender glow accent #B8A0D4',
          'subtle pastel-lavender rim light effect around the border',
        ],
        [
  ...
  'character': [
    {
      ...
      prompt: createCharacterPrompt(
        'rose-quartz toned vest (#E8B4B8) over cream shirt',
        'short spiky warm-brown hair with bold messy silhouette',
        'small sword hilt at shoulder',
        'brave beginner adventurer',
      ),
  ...
  'forest': [
    {
      ...
      prompt: createMonsterPrompt(
        ...
        'small goblin with moss-green skin (#7AB88A)',
        'pointy ears and small rusty knife',
        'hunched sneaky silhouette',
      ),
  ...
  'background': [
    {
      ...
      prompt: createBackgroundPrompt(
        'forest region tier 1 navigation background',
        'soft mint and lavender palette with dreamy forest haze',
        'warm diffused daylight opening into a quiet forest clearing',
        'gentle tree masses and soft foliage shapes with minimal detail',
        'mint, sage green, cream, and lavender atmosphere blend',
        'cozy welcoming woodland mood with beautiful stillness',
      ),
...
faceted gemstone coin face with warm inner glow,
simple centered sun emblem with short radiant rays,
gold luminous core highlight #C9A86C,
polished translucent gemstone surface with subtle refraction,
soft cream highlight on upper edge,
clean circular rim with pastel gold tint,
...
faceted gemstone coin face with cool inner glow,
simple centered crescent moon emblem with two tiny stars,
deep purple luminous core #6A5080,
polished translucent gemstone surface with subtle refraction,
soft lavender-cream highlight on upper edge,
clean circular rim with deep purple tint,
